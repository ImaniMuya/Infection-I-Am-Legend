 ...
 * @author Stefan Brandle edited by Imani Muya and Grace Rose
 * @date November 2017
 */
...
void Board::run() {
   ...
   // Creates 'Human' objects and sets the array pointers to point at them.
    for(int pos=0; pos<numHumans; ++pos) {
        if (pos == numHumans/2) {
            WILL = new willSmith (row,col,false,this, false);
            humans[pos] = WILL;
        }
        else if (pos == numHumans/2 + 1) {
            DOG = new dog (row,col, false, this, false);
            humans[pos] = DOG;
        }
        else if (pos == numHumans/2 +2) {
            BADGUY = new BadGuy (row,col, true, this, false);
            humans[pos] = BADGUY;
        }
        else {
        	...
        	humans[pos] = new Human(row, col, false, this, false);
        }
    }
    // Infect a random human in the range 0 to numHumans-1
    humans[random()%numHumans/2-1]->setInfected();

    for(currentTime=0; terminate == false; ++currentTime) {
	...
	// Tell each human to draw itself on board
	for(int pos=0; pos<numHumans; ++pos) {
	    humans[pos]->draw();
	}
	// Print statistics.
	cout << conio::gotoRowCol(numRows+3, 1)
         ...
         << endl << endl << "Welcome, you are now about to witness I Am Legend in 2 minutes. "
         << endl << "Slowly all of the humans (green squares) are becoming zombies (red squares) "
         << endl << "by coming into contected with zombies. All are susceptible accept for Will Smith, and his dog, "
         << endl << "who cannot be zombiefied. So, Will goes on a murderous rampage killing zombies, making them immobile. "
         << endl << "This is all fine accept for the BadGuy (yellow square), the only one who can kill the dog.  "
         << endl << "The simulation ends when either the BadGuy kills the Dog or Will Smith kills the BadGuy. "
         << endl << "Enjoy" << endl;
...
bool Board::allInfected() {
    for(int i=0; i<numHumans; ++i) {
    	if((humans[i]->isInfected() == false) && (humans[i]!= WILL && humans[i]!= DOG && humans[i]!= BADGUY))  return false;
    }
    if (suddenDeathMode == false){
        suddenDeath();
        suddenDeathMode = true;
    }
    cout << endl << "SUDDEN DEATH ACTIVATED" ;
    return true;
}
    ...
    for( int i=0; i<numHumans; ++i ) {
        for( int j=i+1; j<numHumans; ++j ) {
            if( isNextTo(humans[i], humans[j]) ) {
    	        if( (humans[i]->isInfected() && humans[j]->isInfected() == false) && humans[i]->isDead()==false && humans[j]->isDead() ==false ) {
    		        humans[j]->setInfected();
                }
                else if ( (humans[j]->isInfected() && humans[i]->isInfected() ==false)  && humans[i]->isDead()==false && humans[j]->isDead() ==false ) {
    		              humans[i]->setInfected();
                }
                // if (humans[j]->isInfected() && isNextTo(WILL, humans[j])) humans[j]->kill();

            }
        }
        if (humans[i]->isInfected() && isNextTo(WILL, humans[i])) humans[i]->kill();
        if (humans[i]->isInfected() && isNextTo(WILL, BADGUY)) {
            terminate = true;
            cout << "Will Smith Won the Zombie apocolypes and saved the dog!!" ;
            break;
        }
        if (humans[i]->isInfected() && isNextTo(DOG, BADGUY)) {
            terminate = true;
            cout << "NOOOOO!!!! The Dog is killed by the BigBadZombie!!" ;
            break;
        }
    }
...
/**
 * @brief This function initializes suddenDeathMode by sending the BadGuy and the Dog to play chicken
 */
void Board::suddenDeath() {
    BADGUY->setLocation(10,0);
    DOG->setLocation(10,80);
    BADGUY->setDeath();
    DOG->setDeath();
